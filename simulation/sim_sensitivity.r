...
generateData.nphC <- function(n, 
  ...
  U <- runif(n, 0, 1)
  if(censor == "25%"){
    c0j <- seq(0.054, 0.096, 0.001)
    theta1 = 0.4
    theta2 = 0.1
    Xbeta = theta1*Z1 + theta2*Z2
    C <- (-log(U)*(Xbeta+1)/c0j[age-17]) ^ (1/(Xbeta+1))
  }
  else if (censor == "50%"){
    c0j <- seq(0.15, 0.36, 0.005)
    theta1 = 0.2
    theta2 = -0.2
    Xbeta = theta1*Z1 + theta2*Z2
    C <- (-log(U)*(Xbeta+1)/c0j[age-17]) ^ (1/(Xbeta+1))
...
sim.nphC <- function(censor, L, ns = c(1250, 2500, 5000, 10000), n.sim = 500){
  ...
  for (s in 1:length(ns)){
    ...
    for (iter in 1:n.sim){
      dat <- generateData.nphC(n = ns[s], censor = censor)
...
write.csv(res.final, "res_nphC.csv", row.names = FALSE)
